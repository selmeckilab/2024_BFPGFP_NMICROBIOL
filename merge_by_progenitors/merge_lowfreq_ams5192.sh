...
list=ams5192_polyploids_to_merge.txt
merged=ams5192_tmp.vcf
regions=SC5314_A21_region_file.bed
vcf_file=bfpgfp_ams5192.vcf
...
out_file=BFPGFP_AMS5192_low_freq_annotated.vcf
...
bcftools view "${merged}".gz -R "${regions}" \
  ...
  | bcftools view -i "FORMAT/AF[*] >=0.02" \
